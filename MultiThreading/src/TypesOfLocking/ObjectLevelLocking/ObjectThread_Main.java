...
public class ObjectThread_Main
{
    public static void main (String[] args)
    {
        ...
        Thread t3 = new Thread(threadClass3); // If we create another thread using different object this thread will interfere in t1 and t2.
        t1.start();
        t2.start();
        t3.start();
    }
    ...
    public synchronized void test() {
        ...
        for(int i=0 ;i <100; i++) {
            System.out.println("M in thread" + Thread.currentThread().getName() + "  Value is  " + i);
        }
